...
RSpec.describe "Merchant Dashboard", type: :feature do
  before(:each) do
    ...
    @invoice_1 = Invoice.create!(status: 0, customer_id: @customer_6.id, created_at: Time.now - 1.day)
    @invoice_2 = Invoice.create!(status: 1, customer_id: @customer_2.id, created_at: Time.now - 2.days)
    @invoice_3 = Invoice.create!(status: 1, customer_id: @customer_2.id, created_at: Time.now - 3.days)
    @invoice_4 = Invoice.create!(status: 1, customer_id: @customer_1.id, created_at: Time.now - 4.days)
    @invoice_5 = Invoice.create!(status: 0, customer_id: @customer_4.id)
    @invoice_6 = Invoice.create!(status: 2, customer_id: @customer_6.id)
    @invoice_7 = Invoice.create!(status: 1, customer_id: @customer_3.id)
    @invoice_8 = Invoice.create!(status: 0, customer_id: @customer_5.id)
    
    @transaction_1 = Transaction.create!(credit_card_number: 123456789, credit_card_expiration_date: "11/26", result: 0, invoice_id: @invoice_1.id)
    ...
    @transaction_6 = Transaction.create!(credit_card_number: 123456789, credit_card_expiration_date: "11/26", result: 1, invoice_id: @invoice_6.id)
    ...
    @transaction_8 = Transaction.create!(credit_card_number: 123456789, credit_card_expiration_date: "11/26", result: 0, invoice_id: @invoice_8.id)
      ...
      describe "Merchant Dashboard Statistics - Favorite Customers" do
        ...
        it "shows names of the top five customers" do
          ...
          within("#top-five-#{@customer_1.id}") do
            expect(page).to have_content("John Jacobs")
          ...
          expect(page).to_not have_content("Jessica Simpson")
        end
      end
